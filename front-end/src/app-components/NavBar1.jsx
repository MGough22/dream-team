...
{user === "Guest" ? 
<Box as={Link} to={`/login`} bg="gray.400" p="1vh" color="white">{`Login to access your dream journal`}</Box> :
<Box as={Link} to={`/dreamjournal`} bg="gray.400" p="1vh" color="white">{`${username}'s Dream Journal`}</Box> }
...
{user==="Guest" ? null: 
<Text as={Link} to={`/settings`} bg="gray.700" p="1vh" color="white">Logged in user/settings: Welcome, {username}</Text>}
